...
from utils.semantic_operations import *
...
class SemanticInit(ks.initializers.Initializer):
    def __init__(self, class_count):
        self.class_count = class_count

    def __call__(self, shape, dtype=None, **kwargs):
        prev_semantic = tf.zeros([shape[0], shape[1], shape[2], self.class_count-1])
        other_class = tf.ones([shape[0], shape[1], shape[2], 1])
        semantic_init = tf.concat([prev_semantic, other_class], axis = 3)
        return semantic_init

    def get_config(self):  # To support serialization
        return {"class_count": self.class_count}
...
class SemanticRefiner(ks.layers.Layer):
    ...
    def __init__(self, regularizer_weight=0.0004, num_classes = 9):
    ...
    def call(self, feature_map):
        ...
        for i, convs in enumerate(zip(self.est_s_conv_layers)):
            ...
            for j, (prev, conv) in enumerate(zip(prev_outs, convs)):
                ...
                ### We should apply softmax activation on the last convolutional output (only till channel 8)
...
class SemanticEstimatorLevel(ks.layers.Layer):
    ...
    def build(self, input_shapes):
        ...
        #s_maps_init = tf.zeros_initializer()
        s_maps_init = SemanticInit(self.classes)
        #b, h, w = self.shape[:3]
        #self.prev_f_maps_time = tf.zeros(shape=self.shape)
        #prev_semantic = tf.zeros([b, h, w, self.classes-1])
        #other_class = tf.ones([b, h, w, 1])
        #self.semantic_prev_time = tf.concat([prev_semantic, other_class], axis = 3)
        ...
        with tf.name_scope("SemanticEstimator_lvl"):
            ...
            if prev_semantic_time is None or new_traj[0]:
                ...
                with tf.name_scope("preprocessor"):
                    prev_semantic_time = tf.zeros([b, h, w, self.classes-1])
                    other_class = tf.ones([b, h, w, 1])
                    prev_semantic_time_reproj = tf.concat([prev_semantic_time, other_class], axis = 3)
                    #print("(0) PREV_SEMANTIC_TIME_REPROJ: ",prev_semantic_time_reproj)
                    with tf.name_scope("input_prep"):
                        input_features = [prev_semantic_time_reproj, semantic_prev_lvl]
                ...
                with tf.name_scope("semantic_estimator"):
                    ...
                    if not self.is_training:
                        ...
                        #self.prev_f_maps_time=curr_f_maps_lvl
                        #self.semantic_prev_time=semantic
                        #self.prev_f_maps_time=tf.identity(curr_f_maps_lvl)
                        #self.semantic_prev_time=tf.identity(semantic)
                        
            else:
                with tf.name_scope("preprocessor"):
                    prev_semantic_time_reproj = get_semantic_depth_reproj(prev_semantic_time, curr_depth_lvl, rot, trans, camera)
                    #prev_semantic_time_reproj = get_semantic_reproj(prev_semantic_time, rot, trans, camera)
                    
                    with tf.name_scope("input_prep"):
                        #print("PREV_SEMANTIC_TIME_REPROJ: ",prev_semantic_time_reproj)
                        input_features = [prev_semantic_time_reproj, semantic_prev_lvl]
