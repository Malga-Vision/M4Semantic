...
class CustomCheckpointCallback(ks.callbacks.TerminateOnNaN):
    ...
    def on_train_end(self, logs=None):
        return
class CustomAeroscapesValidationCallback(ks.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        main_command = "python %s" % os.path.join(rel_path, "main.py") + ' --mode=validation ' \
                                              ...
                                              '--db_path_config=%s ' \
                                              '--ckpt_dir=%s ' \
                                              '--records_path=%s ' % (self.cmd.db_path_config, self.cmd.ckpt_dir, os.path.join(rel_path, "data/aeroscapes/test_data"))
        opt_args = ''
        ...
        options = '--seq_len=1 --db_seq_len=1 --arch_depth=%i ' % (self.cmd.arch_depth)
...
class BestCheckpointManager(object):
    ...
    def update_backup(self, perfs):
        ...
        # if nbre of existing backups is smaller than max_keep
            ...
            best_perfs = pandas.concat([best_perfs, df], ignore_index=True)
