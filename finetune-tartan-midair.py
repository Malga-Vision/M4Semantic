...
if __name__ == '__main__':
    ...
    tartanair_params = dl.DataloaderParameters(model_opts.dataloader_settings.db_path_config,
                                           os.path.join(*[cmd.records_path, "tartanair","unstructured", "train_data"]),
                                           4, 4, True)
    tartanair_dataloader = dl.get_loader("tartanair")
    tartanair_dataloader.get_dataset("finetune", tartanair_params, batch_size=cmd.batch_size)
    ...
    joint_dataset_length = tartanair_dataloader.length*2
    #joint_dataset_length = midair_dataloader.length 
    joint_dataset = tf.data.Dataset.sample_from_datasets([tartanair_dataloader.dataset.repeat(), midair_dataloader.dataset.repeat()], weights=[0.5, 0.5], stop_on_empty_dataset=True)
    ...
    model.compile(optimizer=opt, metrics=[tf.keras.metrics.MeanIoU(num_classes = midair_dataloader.class_count)])
